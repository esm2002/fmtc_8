...
class ObstacleDetection:
    ...
    def __init__(self):
        ...
        self.count_p = 0 # Number of consecutive frames in which a parking spot is detected
        self.parking_flag = 0 # Flag indicating whether a parking spot is detected
    ...
    def check_obstacle(self):
        ...
        if obstacle_detected:
            ...
            if self.count > 5: 
                ...
                if indices:
                    min_y_index = min(indices, key=lambda index: self.cluster_coordinates[index][1])
                    #change max to min
                    #self.y_coord = np.arctan(self.cluster_coordinates[max_y_index][1]/self.cluster_coordinates[max_y_index][0])*180/np.pi
                    self.y_coord = self.cluster_coordinates[min_y_index][1]
                    print(self.y_coord)
    ...
    def check_parking_spot(self): #FIXME    
        # Check if the specified conditions for obstacle detection are met
        
        # Check if the number of cluster coordinates and cluster point numbers are equal
        if len(self.cluster_coordinates) == len(self.cluster_point_nums):
            obstacle1_detected = any((1 <= np.sqrt(x**2+y**2) <= 1.9 and x<-1 and 0<y<1 and  num >= 30 and num<=70) for (x, y), num in zip(self.cluster_coordinates, self.cluster_point_nums)) #FIXME
            space_detected = not any((1.9 <= np.sqrt(x**2+y**2) <= 2.8 and x<-1 and 0<y<1 and  num >= 30 and num<=70) for (x, y), num in zip(self.cluster_coordinates, self.cluster_point_nums)) #FIXME
            obstacle2_detected = any((2.8 <= np.sqrt(x**2+y**2) <= 3.7 and x<-1 and 0<y<1 and  num >= 30 and num<=70) for (x, y), num in zip(self.cluster_coordinates, self.cluster_point_nums)) #FIXME
        else:
            obstacle1_detected = False
            space_detected = False
            obstacle2_detected = False

        # If the conditions are met, increment the count and set the parking flag
        if obstacle1_detected and space_detected and obstacle2_detected:
            self.count_p += 1
            # If the count exceeds 5, set the parking flag (5번 연속으로 주차 공간이 감지되면 주차공간이 감지된 것으로 판단)
            # 5는 변경 가능, 0으로 두면 인식되자 마자 flag가 1로 바뀜
            if self.count_p > 5: 
                self.parking_flag = 1
                
                # Extract right-side indices of elements that satisfy the parking spot detected condition
                r_indices = [index for index, value in enumerate(self.cluster_coordinates) if (1 <= np.sqrt(value[0]**2+value[1]**2) <= 2.1  and value[0]<-1 and 0<value[1]<1 and  self.cluster_point_nums[index] >= 10)] #FIXME

                # If there are indices that satisfy the condition, find the index with the maximum y-coordinate
                if r_indices:
                    max_y_index = max(r_indices, key=lambda index: self.cluster_coordinates[index][1])
                    #self.y_coord = np.arctan(self.cluster_coordinates[max_y_index][1]/self.cluster_coordinates[max_y_index][0])*180/np.pi
                    self.y_coord = self.cluster_coordinates[max_y_index][1]
                    print(self.y_coord)

        # If the conditions are not met, reset the count and parking flag            
        else:
            # Reset the count and obstacle flag
            self.count_p = 0
            self.parking_flag = 0
    ...
    def publish_obstacle_flag(self):
        ...
        obstacle_flag_msg.parking_flag = self.parking_flag
