...
class ControlCommand() : 
    ...
    def obstacle_callback(self,msg):
        self.obstacle_detect=msg.data
        print('check: ',self.obstacle_detect)
    ...
    def arduino_command_pub(self): # 아두이노에 최종 명령 publish
        ...
        print('obstacle: ', self.obstacle_detect)


         ###################### 신호등 인식 및 정지 ##################################
        if(self.traffic_ligit_detect_stop == 1 and self.obstacle_detect == 0) : # 신호등 stop && 장애물 미감지
        ...
        ###################### 장애물 + Lane Change ##################################
        if(self.obstacle_detect == 1) : # 장애물 감지 && 신호등 go
            
            self.obstacle_detect = 0 # 장애물 감지 여부 초기화

            # 차선변경 룰베이스
            print("장애물이 감지되었습니다. 룰베이스 차선변경입니다.")

            time_right_tilt = 3.3 # 우회전 시간
            time_left_tilt = 3.3 # 기준 좌회전 시간에 y좌표에 따라 추가 시간
            time_straight = 1 # 직진 최대 시간

            control_msg.data = 48 # 정지 명령 -> 48일때 정지, 20 미만일 때 각도 부여 .. 17 좌회전, -15 우회전
            ...
            print('left') # for debugging, 해당 명령 pub 시간 출력
            start_time = rospy.get_time()
