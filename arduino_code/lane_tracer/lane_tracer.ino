...
void change_angle(int angle){
  if((potentiometer_Read(analogPin) - val_mid) >= (angle + 3)){
    while(1){
    right_tilt();
    if ((potentiometer_Read(analogPin) - val_mid) <= (angle)){
      stop_tilt();
      break;
    }
    }
  ...
  else if((potentiometer_Read(analogPin) - val_mid) <= (angle - 3)){
    while(1){
    left_tilt();
    if ((potentiometer_Read(analogPin) - val_mid) >= (angle)){
      stop_tilt();
...
ros::Subscriber<std_msgs::Int16> sub("lane_decision", motor);
