...
void commandCallback(const std_msgs::Int16& command_msg) {
  ...
  if (command == 1) { // Left tilt + Forward
    change_angle(constrain(current_angle + 5, 5, 20));
    delay(50);
    forward();
  } else if (command == 2) { // Center tilt + Forward
    change_angle(0);
    delay(50);
    forward();
  } else if (command == 3) { // Right tilt + Forward
    change_angle(constrain(current_angle - 5, -20, -5));
    delay(50);
    forward();
