...
class TrafficLightDecoder: 
    ...
    def __init__(self):
        ...
        self.bridge = CvBridge() # 이미지 변환을 위한 브릿지 객체
        
        self.cap_img = []
        self.bbox = []
        self.traffic_light_color = "nothing" # 신호등 정보 처리를 위한 변수들 초기화

        # *********추가된 부분: 신호등 색상을 저장할 스택과 스택 크기
        self.color_stack = []
        self.stack_size = 10  # 스택 크기 (10회)
    ...
    def _bbox_callback(self, msg): # yolo_final.py에서 publish한 topic(color, bbox_info)를 subscribe하기 위한 callback 함수
        ...
        # *********추가된 부분: 스택에 새로운 색상을 추가하고 크기를 유지
        self.color_stack.append(self.traffic_light_color)
        if len(self.color_stack) > self.stack_size:
           self.color_stack.pop(0)
    ...
    def decode_traffic_signal(self): # 신호등 색 판단하여 stop_flag, bbox_size return
        ...
        if len(self.bbox) == 0:  # bbox가 없으면 : 감지된 신호등이 없으면
            ...
            # *********추가된 부분: 스택이 채워졌을 때만 색상 판단
            if len(self.color_stack) == self.stack_size:
                # 스택의 모든 값이 동일한지 확인
                if all(color == 'red' for color in self.color_stack):
                ...
                elif all(color == 'green' for color in self.color_stack):
                    stop_flag = 0  # 이동신호 전달
                    return stop_flag, 0
