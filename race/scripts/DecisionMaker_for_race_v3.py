...
class lane_masking : 
  ...
  def __init__ (self) :
    ...
    rospy.Subscriber('/usb_cam/image_raw/compressed', CompressedImage, self._image_callback) # get image from usb_cam node

    self.steer_pub = rospy.Publisher('Steer_value', Int16, queue_size=10) # define topic
  ...
  def _image_callback(self, msg): # 불러온 img를 cv2 image 로 변환
    ...
    steer_msg = Int16() 
    steer_msg.data = int(steering) # publish할 메세지에 steering 값 부여
