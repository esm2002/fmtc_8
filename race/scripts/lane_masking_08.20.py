...
def lane_detect(image):
    ...
    # 추가한 부분
    previous_steerings.append(steering)
    if len(previous_steerings) > 3:
        previous_steerings.pop(0)
    #
    steering_avg = sum(previous_steerings) / len(previous_steerings)
    #
    ...
    # steering -> steering_avg
    return result_color, steering_avg
